...
/**
 * ThermosiphonWizard — Step 3 for THERMOSIPHON SOLAR (tank on the roof).
 * Replaces the flat ExtrasList for solar_thermosiphon only.
 *
 *   Q1 Home:  Single/Lowset ($0) · Double/Highset (+$450)
 *   Q2 Pitch: Flat · Moderate ($0) · Steep (+$150) · Crazy Steep (+$500)  — image cards
 *             Flat → tilt frame needed (solar ≥10°): on a tilt frame?
 *                 Yes → reusable? Yes=$0 / No=new tilt frame (+$875)
 *                 No  → new tilt frame (+$875)
 *   Q3 Access: Yes / No / Unsure (informational)
 *   Included: Remove old tank & disposal ($0)
 *
 * No Inside/Outside, Concrete base or Isolator (not applicable — roof tank).
 * Costs are HIDDEN here (shown only on the generated quote). Output → extraIds.
 */
...
const CODE = {
  DOUBLE_STOREY: "double_storey_thermosiphon",
  PITCH_STEEP: "pitch_steep_thermosiphon",
  PITCH_CRAZY: "pitch_crazy_steep_thermosiphon",
  TILT_FRAME: "tilt_frame_thermosiphon",
...
export default function ThermosiphonWizard({
  region,
  systemType,
  onChange,
  onCompletionChange,
  ...
  useEffect(() => {
    if (!region || systemType !== "solar_thermosiphon") return;
    setLoading(true);
    fetch(`/api/pricing/extras?region=${region}&type=solar_thermosiphon`)
      .then((res) => res.json())
      .then((data: Extra[]) => {
  ...
  // A new tilt frame is needed on a flat roof when there isn't a reusable one.
  ...
  }, [byCode, home, pitch, needsNewTiltFrame]);
  ...
  const complete = useMemo(() => {
    ...
    return true;
  }, [home, pitch, access, onTiltFrame, tiltReusable]);
  ...
  // Thermosiphon tanks sit on the roof → treat as "outside" for the CRM location.
  useEffect(() => {
      ...
      {home && pitch && (
        <Question title="Is there easy access to your roof?">
          <OptionPills
            ...
            onChange={setAccess}
          />
          {(access === "no" || access === "unsure") && (
            <Note>Roof access will be reviewed on site.</Note>
          )}
        </Question>
      )}
