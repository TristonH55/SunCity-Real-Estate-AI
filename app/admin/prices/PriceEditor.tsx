...
export default function PriceEditor() {
  ...
  const load = (regionCode: string, type: string) => {
    setLoading(true);
    setMessage(null);
    fetch(
  ...
  const save = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Re-baseline: fold edits into the row prices so "changed" clears.
        setRows((prev) =>
          prev.map((r) =>
      ...
      <div className="glass-card p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
          <RegionSelect
            ...
            onChange={(val) => {
              setRegion(val);
              if (systemType) load(val, systemType);
            }}
          />
          <SystemTypeSelect
            value={systemType}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="glass-card w-full max-w-md p-6 space-y-4">
            ...
            <p className="text-sm text-slate-200">
              You are about to update the prices on the <strong>live app</strong>. These changes take
              effect <strong>immediately</strong> for real customer quotes. Are you sure?
            </p>
