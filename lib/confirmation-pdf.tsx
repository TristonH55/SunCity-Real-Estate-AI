...
const styles = StyleSheet.create({
  ...
  systemBox: { borderWidth: 1, borderColor: "#ccc", padding: 10, marginBottom: 14 },
  customerBox: {
    borderWidth: 1,
    borderColor: "#ccc",
    padding: 10,
...
/**
 * Builds the single-order (locked) confirmation PDF bytes. Shared by the
 * agent "View PDF" route and the CRM note attachment. Returns null if the
 * confirmation doesn't exist. No auth — callers guard.
 */
export async function buildConfirmationPdf(
  ...
  const system = await prisma.system.findUnique({
    ...
    select: { brand: true, model: true, capacityLitres: true, tankMaterial: true },
  });
      ...
      <Page size="A4" style={styles.page}>
        ...
        {system && (
          <View style={styles.systemBox}>
              ...
              <View style={styles.systemBox}>
                ...
                {extras.map((extra, index) => (
                  <View key={index} style={styles.row}>
                    ...
                    <Text>
                      ${extra.prices?.[0]?.price ? extra.prices[0].price.toNumber() : 0}
                    </Text>
                  </View>
                ))}
              </View>
            ...
            <Text>Tank: {system.tankMaterial.replace("_", " ")}</Text>
          </View>
        )}
        ...
        <View style={styles.section}>
          ...
          <Text>Date: {confirmation.createdAt.toISOString().slice(0, 10)}</Text>
        </View>
